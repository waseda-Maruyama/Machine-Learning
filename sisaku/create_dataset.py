...
for code_str in prices_df.columns:
    ...
    # バグ修正：インデックスをリセットして結合
    merged_df = pd.merge_asof(
        price_target_df.sort_index().reset_index(), 
        X_fins,                                     
        on='Date',                                  
        direction='backward'
    )
...
if final_dataset.empty:
    ...
    print("\n🎉 V2_Close 公正比較版データセットの作成に成功！")
    output_filename = 'analysis_dataset_v2_close.csv'
