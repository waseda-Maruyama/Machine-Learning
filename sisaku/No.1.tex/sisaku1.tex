...
\subsection*{3. 追加実験：ハイパーパラメータ・チューニング}
モデルの予測精度（46.3\%）が低い原因が、XGBoostのハイパーパラメータ（モデルの
設定値）にある可能性を検証するため、主要なパラメータである\texttt{n\_estimators}
（決定木の数）と\texttt{learning\_rate}（学習率）の値を変更し、
複数パターンで再学習を実行した。

しかし、これらのパラメータをどのように調整しても、テストデータに対する
正解率は46\%台からほぼ変動しなかった。

この結果は、モデルの性能がパラメータの調整不足によるものではなく、
より根本的な問題、すなわち**「入力した特徴量（ファンダメンタルズ）自体に、
短期株価を予測するシグナルがほとんど含まれていない」**ことを強く示唆している。
% ======================================
\section{結果と考察}
% ======================================
\subsection*{1. 予測精度}
