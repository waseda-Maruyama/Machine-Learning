...
SCENARIO_RANGE = ["2024-01-01", "2025-08-30"] # 期間
...
if not os.path.exists(INPUT_FILE1) or not os.path.exists(INPUT_FILE2):
    print(f"❌ ファイルが見つかりません: {INPUT_FILE1} or {INPUT_FILE2}")
    exit()

df_ml_original = pd.read_csv(INPUT_FILE1, index_col=0, parse_dates=True)
df_ml = pd.read_csv(INPUT_FILE2, index_col=0, parse_dates=True)
...
y_test = df_ml_original.loc[test_mask, 'Target']
...
plt.tight_layout()
fname6 = os.path.join(OUTPUT_DIR, f'fig5_13c_micro_data_index2_scenario2_ueda.png')
...
figure6_csv_path = os.path.join(OUTPUT_DIR, f'fig5_13c_micro_data_index2_scenario2_ueda.csv')
...
plt.tight_layout()
fname6 = os.path.join(OUTPUT_DIR, f'fig5_13d_micro_data_index2_scenario2_tarrif.png')
plt.savefig(fname6, dpi=300)
print(f"✅ Saved Figure 6: {fname6}")
plt.close()
...
figure6_csv_path = os.path.join(OUTPUT_DIR, f'fig5_13d_micro_data_index2_scenario2_tarrif.csv')
df_figure6.to_csv(figure6_csv_path, index=False)
print(f"📊 Saved Figure 6 data to: {figure6_csv_path}")