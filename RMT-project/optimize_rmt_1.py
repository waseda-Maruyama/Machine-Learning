...
OUTPUT_DIR = "output_optimization_1"
...
df_base['Target'] = target_a




# 重み生成
sample_weights_train = pd.Series(1.0, index=target_a.index)
if target_a.sum() > 0:
    # 1. 時間減衰 (Time Decay)
...
for rmt_w in tqdm(RMT_WINDOWS, desc="Scanning"):
    ...
    for s_name, (s_start, s_end) in scenarios.items():
        ...
        w_te = df_ml.loc[mask_test, 'Sample_Weight_train']
