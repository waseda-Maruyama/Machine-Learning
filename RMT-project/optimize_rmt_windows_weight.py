...
common_cols = df_close.columns.intersection(df_caps.columns)
df_close_common = df_close[common_cols]
df_caps_common = df_caps[common_cols]

window = 60
P_bar = df_close_common.rolling(window).mean()
sigma = df_close_common.rolling(window).std()

z_scores = (df_close_common - P_bar) / (sigma + 1e-8)

market_total_cap = df_caps_common.sum(axis=1) + 1e-8
w = df_caps_common.div(market_total_cap, axis=0)

per_stock_energy = w * (z_scores**2)
df_base['E_pot'] = per_stock_energy.fillna(0).sum(axis=1)
...
for rmt_w in tqdm(RMT_WINDOWS, desc="Scanning"):
    ...
    feats = ['Return', 'Vol_20', 'Momentum_10', 'RSI_14', 
             'RMT_Level', 'RMT_Vel', 'RMT_Accel', 'E_pot', 'E_pot_Vel', 'E_pot_Accel']
